...
public class Frame extends JFrame {
    ...
    public Frame() {
        ...
        setTitle("圖片上傳與瀏覽 - swingPlusI - 單檔+進度條");
        setSize(950, 650);
    ...
    private void uploadImage() {
         ...
         * 4. 建立 SwingWorker，讓上傳在背景執行。
         * 5. 上傳過程中更新 progressBar。
         * 6. 上傳完成後重新整理左側清單。
         * 7. 右側顯示剛上傳的圖片。
         */
        ...
        if (result == JFileChooser.APPROVE_OPTION) {
            ...
            /*
             * SwingWorker<File, Void> 說明：
             *
             * SwingWorker 是 Java Swing 內建的「背景工作」類別。
             * 它本身是一個泛型類別，不是我們自己寫的泛型。
             *
             * SwingWorker<T, V>
             *
             * T：doInBackground() 最後回傳的型別。
             * V：中途用 publish/process 傳資料時的型別。
             *
             * 這裡寫 SwingWorker<File, Void>：
             *
             * File：
             * - 表示背景工作完成後會回傳 File。
             * - 所以下面的 doInBackground() 回傳型別也是 File。
             *
             * Void：
             * - 表示這個範例沒有使用 publish/process 傳中途資料。
             * - 因為不用第二個型別，所以寫 Void。
             *
             * new SwingWorker<File, Void>() { ... }
             * 是匿名類別寫法：
             * - SwingWorker 是抽象類別，不能直接 new。
             * - 所以這裡建立一個沒有名字的子類別。
             * - 並且覆寫 doInBackground() 和 done()。
             *
             * doInBackground()：
             * - 在背景執行緒執行。
             * - 適合做耗時工作，例如複製檔案。
             ...
             * done()：
             * - 背景工作完成後執行。
             * - 適合更新 Swing 畫面，例如顯示圖片、更新狀態。
             *
             * 如果直接在按鈕事件裡複製大檔案，畫面可能會卡住。
             */
            SwingWorker<File, Void> worker = new SwingWorker<File, Void>() {
                ...
                protected File doInBackground() throws Exception {
                     ...
                     * 這裡收到 progress 後，再呼叫 SwingWorker 的 setProgress(progress)。
                     */
                    ...
                    return uploadService.upload(sourceFile, uploadProgress);
                }

                @Override
                protected void done() {
                    try {
                        // 取得 doInBackground() 最後 return 回來的 File
                        File targetFile = get();
            ...
            /*
             * 監聽 SwingWorker 的 progress 屬性。
             *
             * UploadService 每複製一段資料，就會算出目前百分比，
             * 然後呼叫 IUploadProgress.onProgress(progress)。
             *
             * onProgress 裡面會呼叫 SwingWorker 的 setProgress(progress)。
             *
             * setProgress(progress) 會讓 SwingWorker 發出一個
             * PropertyChangeEvent。
             *
             * 這裡的 PropertyChangeListener 就是在接收這個事件，
             * 然後把 progressBar 更新成新的百分比。
             */
            worker.addPropertyChangeListener(new PropertyChangeListener() {
                @Override
                public void propertyChange(PropertyChangeEvent evt) {
                    // evt.getPropertyName() 可以知道是哪一個屬性改變。
                    // 我們只關心 "progress"，其他屬性變化不處理。
                    if ("progress".equals(evt.getPropertyName())) {

                        // evt.getNewValue() 是新的 progress 值。
                        // SwingWorker 的 progress 是 int，所以轉成 Integer。
